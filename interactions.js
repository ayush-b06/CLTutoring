...
async function introAnim() {
    ...
    if (!sessionStorage.getItem('introPlayed')) {
        ...
        setTimeout(() => {
            ...
            document.querySelector('.title-line').style.transform = 'scaleX(1)';
            document.querySelector('.hp-line').style.transform = 'scaleX(1)';
        }, 1); // set to "1" so resets hp-line and title-line
...
function adjustHomepageForMobile() {
    ...
    if (screenWidth <= 600) {
    ...
    } /* (not necessary b/c alr in html) else {
        homepageText.innerHTML = '';
        ...
        h3.innerHTML = 'PRIVATE <b id="charlotte">TUTORING</b> & PERSONALIZED <b id="charlotte">COURSES</b> FOR STUDENTS LIKE <b id="charlotte">YOU.</b>';
...
navPhoneItems.forEach(item => {
  ...
  item.addEventListener('mouseout', (event) => {
    ...
    document.querySelector('.phone-menu-tools > :first-child').style.transform = 'scale(1.10) translateX(2px)';
...
function applyStylesOnScroll() {
    ...
    const body0Seg2Heading = document.querySelector('.body0-sec2-heading');
    const body0Seg2TitleBox = document.querySelector('.body0-sec2-title-box');
    const body0Seg2Service1Picture = document.querySelector('.body0-section:last-child .service-box:first-child .service-img');
    ...
    if (isInViewport(body0Seg2Service1Picture, 50)) {
        setTimeout(() => {
    ...
    */
    ...
    //stripe
    const stripeHeading = document.querySelector('.stripe-heading');
    const stripeLeft = document.querySelector('.stripe-content-left');
    const stripeLine = document.querySelector('.stripe-line');
    const stripeRight = document.querySelector('.stripe-content-right');

    if (isInViewport(stripeHeading, 50)) {
        setTimeout(() => {
            stripeLeft.style.transform = 'translateX(0)';
            stripeLeft.style.opacity = '1';
            stripeRight.style.transform = 'translateX(0)';
            stripeRight.style.opacity = '1';
            stripeLine.style.opacity = '1';
        }, 0);
    }
